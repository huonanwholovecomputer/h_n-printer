...
- **价格云端同步**：单双面/首页/派送/加急以云端 `pricing.json` 为权威（首页费默认 0.10），本地工具启动/连线/每 5 分钟自动校验同步，小程序 / APP 单价经 `/api/pricing` 同步（不再硬编码）
- **云任务接收**：SocketIO 长连接实时推送 + HTTP 拉取补充，断线自动重连
- **多设备接单唯一接管**：多台设备共连服务器时，仅「启用接单」的设备接收订单（同一时间唯一接管者，被占用时提示接管者名称与所有者）
- **云端订单归属**：绑定用户按绑定成员显示归属，未绑定普通用户显示「普通用户」；云端订单标签页的归属/自打/附加服务来自下单方，本地不可调整
- **设备授权**：收支清算「授权」页绑定本设备所有者（新建标签页默认归属该所有者、默认关闭「管理员自行打印」）并查看所有云端设备在线状态；设备列表可删除历史遗留设备（升级/重装产生的重复 client_id，删除接单设备时自动清空接管）
- **无障碍后台打印**：无障碍订单自动创建的标签页在后台进行，不打断当前操作
- **离线同步**：离线订单暂存本地 SQLite，联网后自动上传，最多重试 5 次
- **主题**：浅色 / 深色 / 跟随系统，写 `theme_settings.json`
- **单实例锁**：同一时间仅允许一个实例运行；重复启动时自动把既有实例的窗口置于前台（Windows 命名互斥体 + 命名事件通知，`single_instance.py`）
- **收支清算**：内置 `finance/settlement.html`，日期流水自动归月、分模块密码（含「授权」独立密码）、多人均摊、云端成员维度统计、设备授权与设备列表
- **在线设备日志收集**：本地工具「日志管理」可一键收集所有在线打印设备的本机日志（后端 `/api/log/collect_all` 中转，SocketIO `request_log`/`logs` 事件聚合回报）
- **本地订单归属**：用户任务列表（历史授权用户 / 管理管理员 / 我的订单）与密钥记录「关联订单」计数并入归属成员的本地打印订单，与收支清算成员口径一致

![收支清算 · 云端数据展示](docs/screenshots/cloud-settlement.jpg)
...
**接单唯一接管（v4.3/v5.22）**：多台设备同时连接服务器时，两条分发路径都只发给「启用接单」的设备（`printer_claim.json` 持久化唯一接管者，`get_active_printer_client()` 判定）。未启用接单的设备不接收订单；页数分析、预约单文件下发同样只走接单设备。后端 `devices.json` 记录每台设备的计算机名 / 所有者绑定 / 在线时间，供收支清算「授权」页展示。
...
PDF 等后端可直接数页的类型：上传时即用 pypdf 数页返回；`/api/file_page` 轮询时若页数缺失会服务端补数，不依赖本地工具（本地工具离线也不显示「无法返回页数」警告，保持等待中）。
...
单面 0.2 元/页，双面 0.3 元/张（每张印两页，奇数页最后一张按单面 0.2 元计费）。附加服务（派送百分比、加急费、首页费）在两端的 config 中都有对应字段，价格以云端 `pricing.json` 为权威（首页费默认 0.10），本地工具定期校验同步。管理员提交的订单 `is_free=1`，不计费。
