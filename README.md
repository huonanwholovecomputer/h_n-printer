...
- **键盘快捷键**：`Ctrl+C` 复制总价、`Ctrl+Shift+C` 复制计费明细、`Delete`/`Ctrl+D` 删除任务
- **自动转换**：所有非 PDF 文件自动转换为 PDF 后再打印；已有文件不重复转换
- **静默打印**：三级降级策略——Windows GDI 原生 API → SumatraPDF → 应用层循环打印，无需弹出打印对话框
- **双面打印**：支持长边/短边翻转
- **页码范围**：精确指定要打印的页面，支持多段范围（如 `1-5,7,10-15`），自动排序，格式错误实时高亮
- **份数控制**：每个文件独立设置打印份数
- **自动计费**：可配置单面/双面单价，自动计算每项任务费用和总费用；一键复制总价/计费明细
